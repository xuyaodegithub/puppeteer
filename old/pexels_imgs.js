...
async function  getDataList(page,browser,navigationPromise){
    ...
    //     page.click('.media_list > .pure-button'),
    // ]);
    console.log(pageNum,allPage,'pppppppppppp')
    const url=await page.$eval('.media_list > .pure-button',el=>el.href)
        await page.goto(url,{timeout:0});
    //  timer=setInterval(async ()=>{
        console.log('timernostop')
        // if(await page.$('#paginator_clone input[name=pagi]') && await page.$eval('#paginator_clone input[name=pagi]',el=>el.value)!==pageNum){
            // clearInterval(timer)
...
async function cilcBtn(page,browser,navigationPromise){
    ...
    console.log(allTypeUrl[type-1],'___________')
     ...
     //    if(await page.$('#paginator_clone input[name=pagi]') && await page.$eval('h1',el=>el.innerText) !== h1Text) {
